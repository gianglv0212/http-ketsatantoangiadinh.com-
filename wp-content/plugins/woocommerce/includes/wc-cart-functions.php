...
function wc_add_to_cart_message( $product_id ) {
	...
	if ( is_array( $product_id ) ) {
		...
		$added_text = sprintf( __( 'Thêm &quot;%s&quot; vào giỏ hàng của bạn.', 'woocommerce' ), join( __( '&quot; and &quot;', 'woocommerce' ), array_filter( array_merge( array( join( '&quot;, &quot;', array_slice( $titles, 0, -1 ) ) ), array_slice( $titles, -1 ) ) ) ) );

	} else {
		$added_text = sprintf( __( '&quot;%s&quot; đã được thêm vào giỏ hàng của bạn.', 'woocommerce' ), get_the_title( $product_id ) );
	}
	...
	if ( get_option( 'woocommerce_cart_redirect_after_add' ) == 'yes' ) :
		...
		$message 	= sprintf('<a href="%s" class="button wc-forward">%s</a> %s', $return_to, __( 'Quay lại trang chủ ', 'woocommerce' ), $added_text );

	else :

		$message 	= sprintf('<a href="%s" class="button wc-forward">%s</a> %s', wc_get_page_permalink( 'cart' ), __( 'Xem giỏ hàng của bạn', 'woocommerce' ), $added_text );
...
function wc_cart_totals_coupon_label( $coupon ) {
	...
	echo apply_filters( 'woocommerce_cart_totals_coupon_label', esc_html( __( 'Mã phiếu giảm giá:', 'woocommerce' ) . ' ' . $coupon->code ), $coupon );
}
