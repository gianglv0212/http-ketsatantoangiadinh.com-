...
} elseif ( class_exists( 'WooCommerce' ) && get_theme_mod( 'e_shop_demo_front_page', 1 ) == 1 ) { // Display demo homepage only if WooCommerce plugin is activated and demo enabled via customizer.
		...
		<div class="rsrc-home">                                   
			<div <?php post_class( 'rsrc-post-content' ); ?>>                                                      
				<div class="entry-content">
					<?php if ( get_theme_mod( 'e_shop_front_page_demo_carousel', 1 ) == 1 ) {
					...
					if ( $loop->have_posts() ) :
						if ( get_theme_mod( 'e_shop_front_page_demo_style', 'style-one' ) == 'style-one' ) {
						...
						<p class="text-center">	
							<?php esc_html_e( 'No products found', 'e-shop' ); ?>
						</p>
					<?php endif; ?>
				</div>                                                       
			</div>        
